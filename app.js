...
async function apiLogin(phone, password, city, district) {
  return apiCall({ action: "login", phone, password, city, district });
}

async function apiGetProfile(phone, password, city, district) {
  return apiCall({ action: "getProfile", phone, password, city, district });
}

async function apiUploadAfterLogin(phone, password, city, district, payload) {
  return apiCall({ action: "uploadAfterLogin", phone, password, city, district, payload });
}


// -------------------- Session helpers --------------------
// 用 localStorage 保存登入狀態（手機/密碼）供 portal.html 使用
function saveSession(phone, password, city, district) {
  localStorage.setItem("join_phone", String(phone || ""));
  localStorage.setItem("join_password", String(password || ""));
  localStorage.setItem("join_city", String(city || ""));
  localStorage.setItem("join_district", String(district || ""));
}
...
function getSession() {
  return {
    ...
    city: localStorage.getItem("join_city") || "",
    district: localStorage.getItem("join_district") || "",
  };
}
...
function clearSession() {
  ...
  localStorage.removeItem("join_city");
  localStorage.removeItem("join_district");
}
